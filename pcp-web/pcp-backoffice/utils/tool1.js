...
myUtil.request = function (param, pathData) {
    return new Promise((resolve, reject) => {
        let key;
        for(key in pathData){
            param.uri = param.uri.replace('{' + key + '}', pathData[key]);
        }

        if(/{/.test(param.uri) || /}/.test(param.uri)){
            reject({errmsg:'路径参数不足', errcode:1234654});
        }else{
            let defaultParam = {
                json : true,
                headers: {
            ...
            let _param = extend(true, {}, defaultParam, param);
            request(_param, (error, response, body) => {
                if(error){
                    reject(error, response);
                }else{
                    if(body && body.success){
                        resolve(body.data || body.value, response);
                    }else{
