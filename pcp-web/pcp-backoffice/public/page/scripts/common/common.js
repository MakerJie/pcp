...
org.breezee.utils = {
    ...
    navUtil: function () {
        $(".btn_password").click(function () {
            let data = Dolphin.form.getValue("#password_form");
            if (data.newPassword != data.confirmPassword) {
                Dolphin.alert("新密码与确认密码不一致");
            } else {
                data.menuCode = 'account';
                data.id = org.breezee.context.userData.userId;
                data.code = org.breezee.context.userData.userCode;
                data.properties = {
                    _password: true
                };
                Dolphin.ajax({
                    url: (org.breezee.context.contextPath == '/' ? '' : org.breezee.context.contextPath) + '/api/044a10ad24a598a787b50a0dc757e06c',
                    type: Dolphin.requestMethod.POST,
                    data: Dolphin.json2string(data),
                    success: function (reData) {
                        if (reData.success) {
                            Dolphin.goUrl("/logout");
                        } else {
        ...
        $(".btn_suggest_save").click(function () {
            let data = Dolphin.form.getValue("#suggest_form");
            Dolphin.ajax({
                url: (org.breezee.context.contextPath == '/' ? '' : org.breezee.context.contextPath) + '/api/bca4d056b127489eb1803dbd84bc86e8',
                type: Dolphin.requestMethod.PUT,
                data: Dolphin.json2string(data),
                success: function (reData) {
                    if (reData.success) {
                        Dolphin.alert('意见提交成功');
                    } else {
                        Dolphin.alert('意见提交失败：'+reData.msg);
                    }
                },
                onError: function (data) {
                    alert(data.error);
...
org.breezee.syncListen = function (info, obj) {
    $(".btn_sync").click(function () {
        Dolphin.confirm(info, {
            callback: function (flag) {
                if (flag === true) {
                    Dolphin.ajax({
