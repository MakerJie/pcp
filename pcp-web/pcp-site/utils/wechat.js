...
const weChatUtil = {
    ...
    sn_user_info_url: 'https://api.weixin.qq.com/sns/userinfo',
    ...
    config: {
        ...
        mch_id: '1316675801',
        secret: '22c92fe717759506ba260382f14def77',
...
weChatUtil.getOpenId = function (code, callback) {
    ...
    request({
        ...
        if (retData.openid) {
            callback(retData.openid, retData.access_token);
        } else {
...
weChatUtil.getToken = function (callback) {
    let _this = this;
    //http://mp.fafa.com.cn:1980/api/getWeChatTokenString?token=226bc338-e7d1-4bf0-8aed-8c23a30eaa9f
    request({
        method: 'get',
        uri: "http://mp.fafa.com.cn:1980/api/getWeChatTokenString?token=226bc338-e7d1-4bf0-8aed-8c23a30eaa9f"
    }, function (error, response, body) {
        if (error) {
            callback("000000");
            return;
        }
        console.log("weChatUtil.getMKToken:@" + body + "@");
        if (body) {
            callback(body);
        } else {
...
weChatUtil.getMKToken = function (callback) {
    let _this = this;
    //http://mp.fafa.com.cn:1980/api/getWeChatTokenString?token=226bc338-e7d1-4bf0-8aed-8c23a30eaa9f
    request({
        method: 'get',
 ...
 * @param calback
 */
weChatUtil.getUserInfo = function (openId, token, calback) {
    let _this = this;
    console.log(token, '==========');
    if (token) {
        request({
            method: 'get',
            uri: _this.sn_user_info_url + "?access_token=" + token + "&openid=" + openId + "&lang=zh_CN"
        }, function (error, response, body) {
            if (error) {
...
weChatUtil.preOrder = function (obj, callback) {
    ...
    params['sign'] = sign;
    let xml = js2xmlparser("xml", params);
    console.log(xml);
    request({
        method: 'post',
